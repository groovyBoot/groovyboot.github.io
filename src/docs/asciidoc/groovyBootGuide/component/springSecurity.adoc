...
[source,groovy]
----
implementation('org.yunchen.gb:gb-plugin-springsecurity:1.1.0')
----
...
gb:
    springsecurity:
      ...
      userLookup:
        userDomainClassName: cn.com.capinfo.bookstore.domain.core.BaseUser
        authorityJoinClassName: cn.com.capinfo.bookstore.domain.core.BaseUserBaseRole
      authority.className: cn.com.capinfo.bookstore.domain.core.BaseRole
      requestMap.className : cn.com.capinfo.bookstore.domain.core.Requestmap
      ...
      successHandler:
        ...
        systemloginRecord: cn.com.capinfo.bookstore.domain.core.SystemLoginRecord
      logout:
        afterLogoutUrl: /
...
使用系统封装的CapinfoSpringSecurityUtils类或CapinfoSpringSecurityService类获取登录用户信息。
...
使用注入的capinfoSpringSecurityService获取当前登录用户:

    BaseUser currentUser=BaseUser.read(capinfoSpringSecurityService.principal.id);

=== 当前用户鉴权操作

使用CapinfoSpringSecurityUtils类进行用户权限鉴别.

    println CapinfoSpringSecurityUtils.getPrincipalAuthorities();
    println CapinfoSpringSecurityUtils.ifAnyGranted("ROLE_USER,ROLE_ADMIN");
    println CapinfoSpringSecurityUtils.ifAllGranted("ROLE_USER,ROLE_ADMIN");
    println CapinfoSpringSecurityUtils.ifNotGranted("ROLE_USER,ROLE_ADMIN");
...
使用spirng security的taglib获取信息

    <%@ taglib uri="http://www.springframework.org/security/tags" prefix="sec" %>
    <!-- 获取当前用户的用户名  -->
    <sec:authentication property="principal.username"></sec:authentication>

详细的演示在/src/main/webapp/WEB-INF/jsp/workspace/index.jsp中的“用户信息”部分中。
...
CapinfoSpringSecurityService类
需要使用@Autowired 注入
    getPrincipal()        获取当前登录principal ，匿名用户为字符串 anonymous
    注意：登录用户为 cn.com.capinfo.springsecurity. userdetails. CoreUser的实例
    getCurrentUser()   获取当前用户实例 （BaseUser）
    encodePassword(String password)
    encodePassword(String password, Object salt = null)  //EA3.5以后此处盐值不再有用,只为版本一致保留此参数
    isLoggedIn()
...
capinfo:
    springsecurity:
      ...
      corsConfig:
        allowCredentials: true # true or false
        allowedOrigins:  '*'  # * or http://localhost:8080,http://capinfo.com.cn
        allowedHeaders:  '*'  #
        allowedMethods:  '*' # GET,POST or *
        corsPath: /**
----

TIP: 本功能自2.4.12 后提供支持
...
TIP: sm4Key也可不设置，则会默认使用内置的32位16进制密钥。版本从3.6.1开始支持此功能。
...
在任何标有@CapinfoBootstrap注解的类中，如Startup类，添加如下方法
...
class BaseUser implements Serializable {
    ...
    protected void encodePassword() {
        ...
        password = CapinfoSpringUtils.getBean("passwordEncoder").encode(coreUser,password)
    }
}
----
...
TIP: 若订阅安全基类AppSecurityEvent事件，则能收到全部EA框架发布的安全事件。
...
EA提供session复制，jwt存储认证的横向扩展能力;从3.7.1版本后，支持cookie存储认证信息的横向扩展能力。

=== session复制

参看 link: https://ea.capinfo.com.cn/document/html5/eaGuide/eaSessionRedis.html[session 复制]

=== JWT方案

参看 link: https://ea.capinfo.com.cn/document/html5/eaGuide/eaSpringSecurityRest.html[EA Spring Security REST 解决方案]
...
在build.gradle中升级 ea springsecurity组件

----
 compile('cn.com.capinfo.ea:capinfoEA-plugin-springsecurity:3.7.1')
----
